...
query::query(int total_label_size, std::vector<int> words, std::vector<int> labels, int null_tag):total_label_size(total_label_size), words(words), labels(labels), null_tag(null_tag) {
	...
	this->viterbi_prob.resize(this->length);
	this->viterbi_back.resize(this->length);
	for (i=0; i<this->length; i++) {
		this->viterbi_prob[i].resize(this->my_label_size, 0.0);
		this->viterbi_back[i].resize(this->my_label_size, 0);
	}
}
